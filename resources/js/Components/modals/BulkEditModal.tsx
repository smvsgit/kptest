...
export default function BulkEditModal({ids,categories,departments,masterData,role,onClose,onSuccess,onError}:Props){
    ...
    const [sourceType,setSourceType]=useState(''),[assetStatus,setAssetStatus]=useState('');
    ...
    const hasChanges=useMemo(()=>Boolean(categoryId||subcategoryId||tagsMode||archiveMode||accessPolicy||downloadMode||departmentId||year||countryId||stateId||cityId||mandirId||eventId||personId||languageId||mediaTypeId||sourceType||assetStatus),[categoryId,subcategoryId,tagsMode,archiveMode,accessPolicy,downloadMode,departmentId,year,countryId,stateId,cityId,mandirId,eventId,personId,languageId,mediaTypeId,sourceType,assetStatus]);
    ...
    const submit=async()=>{
        ...
        if(eventId)metadata.event_id=Number(eventId); if(personId)metadata.person_id=Number(personId); if(languageId)metadata.language_id=Number(languageId); if(mediaTypeId)metadata.media_type_id=Number(mediaTypeId); if(sourceType)metadata.source_type=sourceType; if(assetStatus)metadata.asset_status=assetStatus;
