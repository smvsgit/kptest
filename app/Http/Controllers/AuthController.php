...
class AuthController extends Controller
{
    ...
    public function login(Request $request, AuditService $audit, NetworkPolicyService $network)
    {
        ...
        $email=strtolower(trim($credentials['email']));$user=User::whereRaw('LOWER(email)=?',[$email])->first();if($user?->locked_until&&$user->locked_until->isPast())$user->forceFill(['locked_until'=>null,'failed_login_count'=>0])->save();$settings=SystemSetting::valueFor('security.auth',[]);$limit=max(3,(int)($settings['failed_login_limit']??5));$lock=max(1,(int)($settings['lockout_minutes']??15));
        ...
        if($user && !$network->allows($request,$user)){$audit->log($request,'auth.login-network-blocked',$user,'Login blocked by network/VPN policy.',['ip'=>$request->ip()]);return back()->withErrors(['email'=>'Sign-in is restricted to the approved internal network or VPN.'])->onlyInput('email');}
    ...
    public function register(Request $request)
    {
        ...
        $defaultDepartmentId=Department::where('is_system',true)->value('id');$user=User::create(['name'=>$data['name'],'email'=>$data['email'],'phone'=>$data['phone']??null,'department_id'=>$defaultDepartmentId,'password'=>Hash::make($data['password']),'role'=>'viewer','status'=>'active']);Auth::login($user);$request->session()->regenerate();$request->session()->put('policy_last_activity',time());return redirect()->route('dashboard');
    }

    public function logout(Request $request, AuditService $audit)
    {
        if($request->user())$audit->log($request,'auth.logout',$request->user(),'User logged out.');Auth::logout();$request->session()->invalidate();$request->session()->regenerateToken();return redirect()->route('login');
    }
}
